...
export function ProductCard({ product }: { product: ShapedProduct }) {
  ...
  // For packs, use retailTotal as the crossed-out price
  // For standard products, use originalPrice
  const hasDiscount = isPack 
    ? product.retailTotal > product.defaultPrice
    : product.originalPrice > product.defaultPrice

  const crossedOutPrice = isPack ? product.retailTotal : product.originalPrice
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-xl border border-white/5 bg-[#141414] transition-all duration-300 hover:border-primary/50 hover:shadow-[0_0_15px_rgba(0,0,0,0.5)]">
      <Link href={`/product/${product.slug}`} className="relative aspect-[4/5] w-full overflow-hidden bg-[#1a1a1a]">
        ...
        {isPack && !hot ? (
          <div className="absolute left-3 top-3 z-10">
            <Badge className="rounded-sm bg-purple-600 text-[10px] font-extrabold uppercase tracking-wide text-white">
              Pack
            </Badge>
          </div>
        ) : null}
        ...
        {/* Discount Badge */}
        {hasDiscount && (isPack || product.discountPercent > 0) ? (
          <div className="absolute right-3 top-3 z-10">
            <Badge className="rounded-sm bg-green-600 text-[10px] font-extrabold uppercase tracking-wide text-white">
              {isPack 
                ? `-${Math.round(((product.retailTotal - product.defaultPrice) / product.retailTotal) * 100)}%`
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-1 text-xs text-white/40">
          {isPack 
            ? `${product.packItems.length} produit${product.packItems.length !== 1 ? 's' : ''}`
